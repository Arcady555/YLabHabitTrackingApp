...
public class JdbcRequests {
    ...
    public static String findLogsByParam(String userId, String action, String dateTimeFrom, String dateTimeTo) {
        return "SELECT * FROM cs_log_schema.logs WHERE " + getRequestForFindLogsByParam(userId, action, dateTimeFrom, dateTimeTo);
    }
    ...
    private static String getRequestForFindLogsByParam(String userId, String action, String dateTimeFrom, String dateTimeTo) {
        StringBuilder stringBuilder = new StringBuilder();
        stringBuilder
                .append(!userId.isEmpty() ? " user_id = ? and" : "")
                .append(!action.isEmpty() ? " action = ?" : "")
                .append(!dateTimeFrom.isEmpty() ? " date_time > ? and" : "")
                .append(!dateTimeTo.isEmpty() ? " date_time < ?" : "");
        if (stringBuilder.toString().endsWith("and")) stringBuilder.setLength(stringBuilder.length() - 3);
